// connections.tsx — "External networks" (OpenVPN + Tailscale) for the /app chat surface.
//
// A React port of the vanilla overlay dialog (#ct-connmodal in overlay.js). Self-contained:
// it talks to the sidecar directly and injects its own CSS, so wiring it up is one import
// plus one element in main.tsx. Same tactic as voice.tsx / agents.tsx / todos.tsx.
//
// Endpoints (sidecar, same origin as /app — the router sends BOTH /app* and /_ct/* to the
// same port, /_ct/ stripped; see claude-router targetForPath):
...
//
// Three things here are load-bearing and were learned the hard way in the overlay:
//
...
//  3. An "Applying…" cover with a blurred backdrop hides the dialog from submit until the
...
//
// The whole feature is INERT unless the server reports enabled:true (cfg.netApplyHelper is
// unset on a vanilla install, and guests are 403'd), so the entry point must stay hidden.
...
// Cached module-wide: the answer cannot change without a server restart, and several
// mount points (button, modal) must not each hit the sidecar.
let availCache: boolean | null = null;
let availInFlight: Promise<boolean> | null = null;

function probeEnabled(): Promise<boolean> {
  if (availCache !== null) return Promise.resolve(availCache);
  if (!availInFlight) {
    availInFlight = fetchConnections()
      .then((d) => {
        availCache = !!(d && d.enabled);
        return availCache;
      })
      .catch(() => false)
...
/** True only when the server reports the network feature is enabled. Hide the entry point otherwise. */
export function useConnectionsEnabled(): boolean {
  const [on, setOn] = useState(availCache === true);
  useEffect(() => {
    if (availCache !== null) { setOn(availCache); return; }
    let live = true;
    void probeEnabled().then((v) => { if (live) setOn(v); });
    return () => { live = false; };
  }, []);
  return on;
}
...
export function ConnectionsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [applying, setApplying] = useState<string | null>(null); // spinner message, null = idle
  ...
  const dataRef = useRef<ConnState>(data);
  ...
  // terminal and this sidecar. Cover the dialog and hold it until the POLLED state reflects
  // the change, or until the safety timeout.
  const beginApply = useCallback((msg: string, check: (d: ConnState) => boolean) => {
  ...
  // releases the dialog instead of spinning forever.
  useEffect(() => {
    if (!applying) return;
  ...
  // Live refresh. Failures (mid-reconnect) are ignored so the spinner holds instead of
  ...
  useEffect(() => {
    if (!open) return;
    setForm(null); setConfirmId(null); setErr("");
    pending.current = null; setApplying(null);
  }, [open]);

  // Escape closes, unless a change is applying (closing would kill the poll that resolves it).
  useEffect(() => {
    if (!open) return;
    const onKey = (e: KeyboardEvent) => { if (e.key === "Escape" && !pending.current) { e.stopPropagation(); onClose(); } };
  ...
  return (
    <div className="cx-scrim" onClick={() => { if (!pending.current) onClose(); }}>
      <div className="cx-panel" role="dialog" aria-modal="true" aria-label="External networks" onClick={(e) => e.stopPropagation()}>
...
/**
 * The whole feature in one element: a globe button that renders NOTHING until the server
 * reports the feature enabled, plus the dialog it opens. Pass a className to borrow an
 * existing button style (e.g. "sb-gear" for the sidebar header).
 */
export function ConnectionsButton({ className = "sb-gear" }: { className?: string }) {
  const enabled = useConnectionsEnabled();
  const [open, setOpen] = useState(false);
  if (!enabled) return null;
  return (
    <>
      <button className={className} onClick={() => setOpen(true)} aria-label="External networks" title="External networks (VPN / Tailscale)">
        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
          <circle cx="12" cy="12" r="9" /><path d="M3 12h18" /><path d="M12 3a15 15 0 0 1 0 18a15 15 0 0 1 0-18z" />
        </svg>
      </button>
      <ConnectionsModal open={open} onClose={() => setOpen(false)} />
    </>
  );
}
// #endregion
...
function injectConnCss() {
  ...
  .cx-panel{position:relative;width:100%;max-width:560px;max-height:84vh;display:flex;flex-direction:column;background:var(--bg-2,#211c18);color:var(--text,#ece7e1);border:1px solid var(--line,#3a322c);border-radius:14px;box-shadow:0 20px 60px rgba(0,0,0,.55);overflow:hidden}
  ...
  .cx-body{overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;padding:16px 18px 20px}
  ...
  .cx-ic{background:transparent;border:none;color:var(--text-3,#8a8078);font-size:12px;padding:4px 7px;border-radius:7px}
